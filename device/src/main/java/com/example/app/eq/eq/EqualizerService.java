...
import com.example.eq.EqualizerImpl;
                ...
                switch(msg.what) {
                    ...
                    case GET_CLIENT_STATUS: {
                        ...
                        if(status == SocketConnector.STATUS_CONNECTED) {
                            ...
                            mReceiver = null;
                            if(mSender!= null) {
                                mSocketConnector.disconnect(mSender);
                            }
                    ...
                    case GET_REMOTE_STATUS: {
                        ...
                        if(status == SocketConnector.STATUS_CONNECTED) {
                            ...
                            mSender = null;
                            if(mReceiver!=null) {
                                mSocketConnector.disconnect(mReceiver);
                            }
                        }
                        if(mSender==null && mReceiver==null) {
                            mUDPBroadcast.enableReceiver(1000);
                    ...
                    case CONNECT_TO_REMOTE: {
                        String ip = msg.getData().getString("ip");
                        int port = msg.getData().getInt("port", -1);
                        if(mSender == null) {
                            mSocketConnector.connect(ip, port);
                        } else {
                            Log.w(TAG, "remote has already connected ("+mSender+")");
                        }
                        break;
                    }
        ...
        for(JSONObject s : request) {
            ...
            if((topic =s.optString("topic")) != null) {
                ...
                if(topic.equals("QueryHost")) {
                    ...
                    //Log.i(TAG, "get TcpConnect ip="+ip1+" port="+port1);
                    if(ip1!=null && port1!=-1) {
